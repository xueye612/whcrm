...
/** image 下载 */
import { downloadFileAPI, downloadFileImageAPI } from '@/api/common'
...
export function getImageData(url) {
  return new Promise((resolve, reject) => {
    ...
    if (url.indexOf('adminFile/down/') !== -1) {
      ...
      request = downloadFileImageAPI
    }
    request({ path: url }).then(res => {
...
export function downloadFile(data) {
  downloadFileAPI({ save_name: data.save_name }).then(res => {
    const blob = new Blob([res.data], {
      type: ''
    })
...
export function getBaiduMap() {
  if (global.BMap && !global.BMap._preloader) {
  ...
  const baiduKey = (global.WKConfig && global.WKConfig.baiduKey) || (typeof WKConfig !== 'undefined' ? WKConfig.baiduKey : '')
  if (!baiduKey || baiduKey === '百度key') {
    return Promise.reject(new Error('BAIDU_MAP_KEY_MISSING'))
  }
  global.BMap._preloader = new Promise((resolve, reject) => {
    ...
    $script.onerror = function() {
      cleanup(new Error('BAIDU_MAP_LOAD_FAILED'))
    }
    $script.src = `https://api.map.baidu.com/api?v=3.0&ak=${baiduKey}&callback=_initBaiduMap`
