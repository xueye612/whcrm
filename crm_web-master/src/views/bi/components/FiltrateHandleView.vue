      ...
      crmBusinessStatusListAPI()
        .then(res => {
          ...
          if (this.businessOptions.length > 0) {
            this.businessStatusValue = this.businessOptions[0].type_id
          }
          result(true)
        })
        .catch(() => {
    ...
    postFiltrateValue() {
      ...
      if (this.showUserStrucSelect) {
        if (this.dataSelect == 1) {
          params.structure_id = this.structuresSelectValue.length > 0
            ? this.structuresSelectValue[0].id
            : (this.safeUserInfo.structure_id || '')
        }
      } else {
      ...
      if (this.showUserSelect) {
        if (this.dataSelect == 2) {
          params.user_id = this.userSelectValue.length > 0 ? this.userSelectValue[0].id : ''
        }
      }
      ...
      if (this.showYearSelect) {
        ...
        if (this.timeTypeValue.type == 'custom') {
          params.start_time = this.timeTypeValue.startTime.replace(/\./g, '-')
          params.end_time = this.timeTypeValue.endTime.replace(/\./g, '-')
        } else {
