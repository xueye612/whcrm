          ...
          <template v-for="(item, index) in seciton.list">
            ...
            <div
              ...
              <div
                v-if="item.type == 2"
                class="activity-cell">
                <span class="activity-cell__label">{{ item.update_time }} {{ item.create_user_name }}创建了{{ getActivityTypeName(item.activity_type) }}：</span><span
                  class="activity-cell__content"
                  @click="checkCRMDetail(getActivityType(item.activity_type), item.activity_type_id)">{{ item.activity_type_name || '查看详情' }}</span>
              </div>
              <div
                v-else-if="item.type == 3"
                class="activity-cell">
                <span class="activity-cell__label">{{ item.update_time }} {{ item.create_user_name }}将商机：</span>
                <span
                  class="activity-cell__content"
                  @click="checkCRMDetail('business', item.activity_type_id)">{{ item.activity_type_name }}</span>
              ...
              <log-cell
                ...
                @delete="logCellDelete" />
              <i
                :class="getActivityIcon(item.activity_type)"
                :style="{ backgroundColor: getActivityTypeColor(item.activity_type) }"
...
export default {
  ...
  computed: {
    ...
    canCreateFollowRecord() {
      return this.crm && this.crm.activity && this.crm.activity.save
    },
    ...
    taskParams() {
      const params = {}
      params[`${this.crmType}_ids`] = this.id
      return params
    },
  ...
  methods: {
    ...
    sendLog(data) {
      ...
      const params = {
        category: data.followType,
        activity_type: crmTypeModel.keyToTypeData[this.crmType],
        activity_type_id: data.id,
        content: data.content,
        next_time: data.nextTime || '',
        is_event: 0,
        file_id: [...data.files.map(_ => _.file_id), ...data.images.map(_ => _.file_id)],
        business_ids: [...data.business.map(item => item.business_id)],
        contacts_ids: data.contactsId
        // types: crmTypeModel[this.crmType],
        // type_id: data.id,
        // content: data.content,
        // category: data.followType
      }
      // params.activityType = crmTypeModel.keyToTypeData[this.crmType]
    ...
    getLogList() {
      ...
      const params = {
        ...
        // activity_type: crmTypeModel.keyToTypeData[this.crmType],
        activity_type: this.activityType.command || '',
      ...
      if (this.filterValue.timeLine.type) {
        if (this.filterValue.timeLine.type === 'custom') {
          params.start_date = this.filterValue.timeLine.startTime.replace(/\./g, '-')
          params.end_date = this.filterValue.timeLine.endTime.replace(/\./g, '-')
        } else {
          params.interval_day = this.filterValue.timeLine.value || ''
        }
      }
