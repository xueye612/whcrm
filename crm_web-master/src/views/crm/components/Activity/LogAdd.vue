...
export default {
  ...
  watch: {
    ...
    id: {
      handler() {
        this.businessAction = { type: 'condition', data: { moduleType: this.crmType, customer_id: this.id }}
      },
      immediate: true
    }
  },
      ...
      this.$confirm('确定删除?', '提示', {
        ...
        .then(() => {
          crmFileDeleteAPI({
            module: 'crm_' + this.crmType,
            module_id: this.id,
            save_name: type == 'image' ? this.imgFiles[index].save_name : this.files[index].save_name
          })
            .then(res => {
              if (type == 'image') {
