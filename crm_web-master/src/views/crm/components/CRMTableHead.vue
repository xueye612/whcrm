  ...
  methods: {
    ...
    selectionBarClick(type) {
      ...
      if (type == 'transfer') {
        ...
        if (this.isSeas) {
          ...
          params[`${this.crmType}_id`] = this.selectionList
    ...
    confirmHandle(type) {
      if (type === 'lock' || type === 'unlock') {
        this.loading = true
        crmCustomerLockAPI({
          is_lock: type === 'lock' ? 1 : 2, // 2是正常 1 是锁定
        ...
        this.loading = true
        crmLeadsTransformAPI({ [this.crmType + '_id']: this.selectionList.map(item => item[this.crmType + '_id']) })
          .then(res => {
            this.loading = false
            this.$message({
              type: 'success',
        ...
        this.loading = true
        crmProductStatusAPI({
          id: this.selectionList.map(item => item.product_id),
        ...
        const id = this.selectionList.map(item => item[this.crmType + '_id'])
        const request = {
