  ...
  <div v-loading="loading" class="rc-cont">
    ...
    <el-dialog
      ...
      <div class="handle-box">
        ...
        <flexbox class="handle-item">
          <div class="handle-item-name">特殊权限：</div>
          <el-checkbox v-model="financeAuth">查看收支</el-checkbox>
        </flexbox>
        <flexbox
          class="handle-item">
...
export default {
  ...
  mounted() {
    ...
    this.fieldList.push({ prop: 'finance_auth_name', width: '200', label: '财务权限' })
  ...
  methods: {
      ...
      crmCustomerTeamMembersAPI(params)
        .then(res => {
          ...
          this.list = (res.data || []).map(item => {
            return {
              ...item,
              finance_auth_name: item.finance_auth == 1 ? '可查看金额' : '不可查看金额'
            }
          })
        })
        .catch(() => {
    ...
    handleClick(type) {
      if (type == 'add') {
        ...
        if (this.selectionList.length == 0) {
          ...
          if (type == 'edit') {
            this.handleType = 1
            this.financeAuth = this.selectionList.every(item => item.finance_auth == 1)
            this.validType = ''
            this.target_time = ''
    ...
    handleEditConfirm() {
      ...
      crmCustomerUpdateMembersAPI({
        ...
        finance_auth: this.financeAuth ? 1 : 0
