  ...
  computed: {
    ...
    name() {
      if (this.crmType === 'receivables') {
        ...
        return this.detail.essential.invoice_apple_number
      }
      return this.detail.name
    },
    // 展示转移
  ...
  methods: {
    ...
    handleTypeClick(type) {
      if (type == 'edit') {
        ...
        // 打印
        const id = this.detail[`${this.crmType}_id`]
    ...
    confirmHandle(type) {
      if (type === 'lock' || type === 'unlock') {
        crmCustomerLockAPI({
          is_lock: type === 'lock' ? '1' : '2', // 1是正常 2 是锁定
          customer_id: [this.id]
        })
          .then(res => {
            this.$message({
        ...
        crmLeadsTransformAPI({ [this.crmType + '_id']: [this.id] })
          .then(res => {
            this.$message({
              type: 'success',
              message: '转化成功'
        ...
        request(this.isSeas ? params : { id: [...params.id] })
          .then(res => {
            this.$message({
              type: 'success',
              message: '删除成功'
    ...
    whetherTypeShowByPermision(type) {
      if (type == 'transfer') {
        ...
        // 合同作废
        if (this.crm[this.crmType].cancel && this.detail.check_status === 2) {
          return true
        }
        return false
      } else if (type == 'print') {
