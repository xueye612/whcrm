        ...
        <div class="detail">
          <flexbox class="detail-images">
            <div v-for="(item, index) in primaryList" :key="index" class="show-img">
              <img
                v-src="item.path||item.file_path"
                :key="item.path">
              <div
                class="img-model">
                <i
...
export default {
  ...
  methods: {
    ...
    deleteImg(type, index) {
      const params = {}
      if (type == 'main') {
        params.file_id = this.primaryList[index].file_id
        params.save_name = this.primaryList[index].save_name
      } else {
        params.file_id = this.detailList[index].file_id
        params.save_name = this.detailList[index].save_name
      }
