...
export default {
  ...
  computed: {
    ...
    allItemsObj() {
      ...
      if (this.oa && this.oa.book && this.oa.book.index) {
        tempsItems.book = {
          title: '通讯录',
          type: 6,
          module: 'book',
  ...
  methods: {
    ...
    getTopHeaderModule() {
      if (this.headerModule.length) {
        const allList = []
        if (this.allItemsObj.workbench) {
          allList.push(this.allItemsObj.workbench)
        }
        for (let index = 0; index < this.headerModule.length; index++) {
          const key = this.headerModule[index]
          const menu = this.allItemsObj[key]
          if (menu && !allList.some(item => item.module === menu.module) && allList.length < 8) {
            allList.push(menu)
          }
        }
        this.items = allList
