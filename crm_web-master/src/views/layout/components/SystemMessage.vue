  ...
  methods: {
    ...
    checkCRMDetail(type, id, dataIndex, data) {
      if (type === 'schedule') {
        this.relationID = id
        if (data.content) {
          this.todayDetailData = data.content
        }
     ...
     */
    downloadError(data) {
      crmDownImportErrorAPI({ name: '导入错误数据', path: data.error_file_path })
        .then(res => {
    ...
    readMessageClick(messageId, index) {
      systemMessageReadAPI({ message_id: messageId })
        .then(res => {
    ...
    deleteMessageClick(messageId, index) {
      this.$confirm('确定删除这条消息?', '提示', {
        ...
        .then(() => {
          systemMessageDeleteByIdAPI({ message_id: messageId })
            .then(res => {
