    ...
    <el-dialog
      ...
      <div v-if="!showRepeat">
        <el-form ref="ruleForm" :model="form" :rules="rules" label-position="left">
          ...
          <el-form-item label="日程类型" prop="schedule_id">
            ...
            <el-select
              ...
              <el-option
                ...
                <span class="custom_right">{{ item.name }}</span>
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="参与人员" prop="owner_user_ids">
            <xh-user-cell
              :value="checkedUser"
              :radio="false"
  ...
  watch: {
    showCreate(val) {
      ...
      if (val) {
        ...
        this.checkedUser = [
          { id: this.userInfo.id, realname: this.userInfo.realname }
        ]
        this.form.owner_user_ids = [this.userInfo.id]
        if (this.selectDiv) {
          this.form.start_time = this.selectDiv + ' 08:00:00'
          this.changeStartTime(this.form.start_time)
        } else {
    ...
    allData: {
      handler(val) {
        Object.keys(val).forEach(key => {
          ...
          const ids = []
          list.forEach(item => {
            ids.push(item[key + '_id'])
          })
          this.businessRelation[key + '_ids'] = ids
        })
      },
      deep: true
    }
  ...
  methods: {
    ...
    getDetail() {
      ...
      if (length !== 0) {
        this.form = {
          ...
          schedule_id: this.todayDetailData.type_id || this.todayDetailData.schedule_id, // 日程类型
          owner_user_ids: this.todayDetailData.owner_user_info,
          repetitionType: this.todayDetailData.repetitionType || '',
          start_time: this.todayDetailData.start_time || '',
          end_time: this.todayDetailData.end_time || '',
          event_id: this.todayDetailData.event_id
        ...
        this.notice = this.todayDetailData.notice
        ...
        this.checkedUser = this.todayDetailData.owner_user_info
        this.cusCheck.forEach(item => {
          if (item.type_id === this.form.schedule_id) {
    ...
    selectUser(data) {
      ...
      if (data.value.length) {
        this.form.owner_user_ids = data.value.map(item => {
          return item.id
        })
        // .join(',')
      } else {
        this.form.owner_user_ids = ''
      }
    },
    ...
    checkInfos(dataIds, data) {
      ...
      Object.keys(dataIds).forEach(key => {
        this.businessRelation[key] = dataIds[key].length ? dataIds[key] : ''
      })
    ...
    createSchedule() {
      ...
      // }
      params = { ...this.form, owner_user_ids: typeof this.form.owner_user_ids[0] == 'number' ? this.form.owner_user_ids : this.form.owner_user_ids.map(item => item.id) }
      for (const key in this.allData) {
        params[key + '_ids'] = this.allData[key].map(item => item[key + '_id'] || item[key + '_id'])
      }
      const length = Object.keys(this.todayDetailData).length
      const request = this.getRequest()
      params.notice = this.notice
      if (length) {
        params.time = new Date(this.todayDetailData.start_time).getTime()
        if (this.editAll) {
          params.type = 3
        } else {
          params.type = 2
