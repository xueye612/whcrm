...
export default {
  ...
  mounted() {
    ...
    allData.business = (this.formData.businessList || []).map(item => {
      if (item.id) {
        item.business_id = item.id || item.business_id
      }
      return item
    })
    allData.contacts = (this.formData.contactsList || []).map(item => {
      if (item.id) {
        item.contacts_id = item.id || item.contacts_id
      }
      return item
    })
    allData.contract = (this.formData.contractList || []).map(item => {
      if (item.id) {
        item.contract_id = item.id || item.contract_id
      }
      return item
    })
    allData.customer = (this.formData.customerList || []).map(item => {
      if (item.id) {
        item.customer_id = item.id || item.customer_id
      }
      return item
    })
    this.allData = allData
    var relevanceAll = {}
    relevanceAll.business_ids = allData.business.map(item => item.business_id)
    relevanceAll.contacts_ids = allData.contacts.map(item => item.contacts_id)
    relevanceAll.contract_ids = allData.contract.map(item => item.contract_id)
    relevanceAll.customer_ids = allData.customer.map(item => item.customer_id)
