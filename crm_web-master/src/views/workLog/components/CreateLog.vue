  ...
  <div
    ...
    <span
      ...
      <i
        ...
        style="color: #2362FB " />
      <span class="complete-btn__name">{{ activeMap[activeTab] }}完成情况<span class="value">{{ completeInfo.endCount }}</span>/<span class="value">{{ completeInfo.logCount }}</span></span>
    </span>
  ...
  methods: {
    ...
    handleCreate() {
      ...
      for (const key in this.relateData) {
        relateData[`${key}_ids`] = this.relateData[key].map(item => {
          return item[`${key}_id`]
        })
      }
      ...
      const params = {
        ...
        send_user_ids: this.sendUserList.map(item => {
          return item.id
        }),
        ...relateData,
        category_id: this.categoryIdMap[this.activeTab],
        file: [...this.files.map(item => item.file_id), ...this.imgFiles.map(item => item.file_id)]
      }

      if (this.isRelevanceReport) {
        params.is_relation = 1 // 1为关联销售简报
      }
