<template>
  <div class="log-item">
    <div class="main">
      <div class="user">
        <xr-avatar
          v-if="data.create_user_info"
          :name="data.create_user_info.realname"
          :id="data.create_user_info.id"
          :size="38"
          :src="data.create_user_info.thumb_img"
          :disabled="false"
          class="user-imgList" />
        <div class="box">
          <div class="username">
          ...
          <div class="time">
            创建日志于 {{ data.create_time }} <!--{{ data.isRead === 1 ? '已读' : '未读' }}-->
          </div>
          <div
            class="handle">
            <el-button
          ...
          <div class="comment-status">
            ...
            <span>{{ getCategory(data.category_id) }}-{{ data.replyList.length === 0 ? '未点评' : '已点评' }}</span>
      ...
      <div v-if="data.bulletin" class="content">
...
export default {
  ...
  computed: {
    ...
    replyTotal() {
      ...
      return num || this.data.replyList.length
    },
    // 日志标题
    logTitleName() {
      return {
    ...
    handleCommand(action, index) {
      if (action === 'delete') {
        this.$confirm('确定删除?', '提示', {
          ...
          journalDeleteAPI({ log_id: this.data.log_id }).then(() => {
