...
export default {
  ...
  computed: {
    ...
    nickName() {
      if (!this.safeUserInfo.realname) return this.safeUserInfo.username
      const reg = /[a-zA-Z]+/
      if (reg.test(this.safeUserInfo.realname) || !this.safeUserInfo.sex) {
        return this.safeUserInfo.realname
      }
      return this.safeUserInfo.realname.slice(0, 1) + (this.safeUserInfo.sex == '男' ? '先生' : '女士')
    }
  },
  watch: {
    filterForm: {
