      ...
      <div
        ...
        <div class="role-nav-box">
          <div
            ...
            {{ item.title }}
            <div
              v-if="item.remark != '超级管理员角色' && item.remark != '项目管理员'"
              class="icon-close">
              <el-dropdown
                trigger="click"
...
export default {
  ...
  computed: {
    ...
    showRuleSet() {
      if (this.roleActive) {
        return (
          this.roleActive.remark != '超级管理员角色' &&
          this.roleActive.remark != '项目管理员'
        )
      }
    ...
    showReminder() {
      if (this.roleActive) {
        return this.roleActive.remark == '项目管理员'
      }
  ...
  methods: {
     ...
     */
    getRulesList() {
      systemRuleByTypeAPI({ pid: this.pid, type: 'tree' }).then(res => {
        if (res.data) {
          this.ruleMenuList = [
            {
              label: '模块功能',
    ...
    getRoleList() {
      ...
      systemRoleByTypeAPI({ pid: this.pid, rules: 1 })
        .then(res => {
     ...
     */
    getReminderContent() {
      if (this.roleActive && this.roleActive.remark == '项目管理员') {
    ...
    ticketsBtn(val) {
      this.$confirm('确定此操作?', '提示', {
        ...
        .then(() => {
          roleCopyAPI({ id: val.id }).then(res => {
    ...
    roleDelect(val) {
      this.$confirm('此操作将永久删除是否继续?', '提示', {
        ...
        .then(() => {
          roleDeleteAPI({ id: val.id }).then(res => {
