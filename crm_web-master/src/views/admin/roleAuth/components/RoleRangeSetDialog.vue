    ...
    <el-checkbox-group v-model="checkList" @change="groupChange">
      <div v-for="(item, index) in treeList" :key="index" class="role-set-section">
        ...
        <div class="role-set-section__list">
          <el-checkbox
            v-for="(subItem, subIndex) in item.item" :key="subIndex"
            :label="subItem.id">{{ subItem.title }}</el-checkbox>
        </div>
      </div>
    </el-checkbox-group>
    <span
...
export default {
  ...
  methods: {
    ...
    getRoleRangeValue() {
      this.loading = true
      adminRoleQueryAuthRoleAPI({ group_id: this.id }).then(res => {
