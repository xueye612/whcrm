        ...
        <div
          ...
          <el-form
            ...
            <el-form-item
              ...
              <el-input
                v-if="item.data.formType == 'textarea'"
                v-model="item.value"
                :autosize="{ minRows: 3}"
                :maxlength="200"
                type="textarea"
...
import { crmExamineSaveAPI } from '@/api/examine'
...
export default {
  ...
  mounted() {
    ...
    if (this.handle.data) {
      // data 存在就处理 save
      if (
        this.handle.data.config &&
        this.handle.data.config === 1
      ) {
        // 固定审批流
        this.config = 1
        this.examineList = []
  ...
  methods: {
    ...
    getField() {
      ...
      field.push({
        ...
        isNull: 0,
        name: '关联对象',
        setting: categoryTypeList,
        // value: this.handle.data ? this.handle.data.categoryType : 'crm_contract'
    ...
    submiteParams(array) {
      ...
      if (this.handle.action == 'update') {
        params.flow_id = this.handle.data.flow_id
      }
      console.log('save: ', params)
      crmExamineSaveAPI(params).then(res => {
        this.loading = false
        this.hidenView()
    ...
    getSubmiteParams(array) {
      ...
      for (let index = 0; index < array.length; index++) {
        ...
        if (element.key === 'dept') {
          if (element.value['users']) {
            params['user_ids'] = element.value['users'] && element.value['users'].map(item => item.id)
          }
          if (element.value['strucs']) {
            params['structure_ids'] = element.value['strucs'] && element.value['strucs'].map(item => item.id)
          }
        } else {
      ...
      params['types'] = params.categoryType
      params['step'] = steps
      return params
    },
