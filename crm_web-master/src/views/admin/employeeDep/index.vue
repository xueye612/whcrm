  ...
  data() {
    return {
      ...
      dialogRules: {
        ...
        groupids: [{ required: true, message: '角色不能为空', trigger: 'change' }]
      },
      // 重置登录账号
    ...
    tableList: function() {
      if (this.dialogTitle === '新建员工') {
        return [
          ...
          { field: 'groupids', value: '角色', type: 'selectCheckout' }
        ]
      } else {
        return [
          {
    ...
    getUserList() {
      ...
      if (this.currentMenuData) {
        ...
        if (this.currentMenuData.type) {
          ...
          params.structure_id = this.currentMenuData.id
          params.status = 'all'
        }
      }
      usersListIndexAPI(params)
        .then(res => {
      ...
      this.$refs.dialogRef.validate(valid => {
        if (valid) {
          if (this.dialogTitle == '新建员工') {
            ...
            formInline.group_id = formInline.groupids
            formInline.structure_id = formInline.structure_id
            formInline.parent_id = formInline.parent_id
            ...
            this.loading = true
            const formInline = objDeepCopy(this.formInline)
            // formInline.groupids = formInline.groupids.join(',')
            formInline.group_id = formInline.groupids
            formInline.structure_id = formInline.structure_id
            formInline.parent_id = formInline.parent_id
      ...
      this.$refs.passForm.validate(valid => {
        if (valid) {
          ...
          if (this.selectionList.length > 0) {
            id = this.selectionList
              .map(function(item, index, array) {
                return item.id
              })
          } else {
            id.push(this.dialogData.id)
          }
          val.id = id
          this.loading = true
...
.new-dialog-form ::v-deep .el-select {
  display: block;
}
