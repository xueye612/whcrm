...
export default {
  ...
  created() {
    if ((this.userShow && this.selectionList.length > 0) || (!this.userShow && this.selectionList.length === 1)) {
      ...
      this.ruleForm.roleList = userData.groupids ? this.selectionList[0].groupids.split(',').map(item => parseFloat(item)) : []
    }
  },
  methods: {
    close() {
    ...
    sureClick() {
      this.$refs.editRoleForm.validate((valid) => {
        if (valid) {
          ...
          if (this.userShow) {
            params.user_id = this.ruleForm.userIds
            params.structure_id = this.ruleForm.deptIds
          } else {
            params.user_id = this.selectionList.map(item => item.id)
          }
