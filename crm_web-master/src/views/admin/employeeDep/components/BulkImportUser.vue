    ...
    <div class="dialog-body">
      ...
      <div
        ...
        <div class="result-info">
          ...
          <p class="result-info__detail">导入总数据<span class="result-info__detail--all">{{ resultData.total }}</span>条，导入成功<span class="result-info__detail--suc">{{ resultData.total - (resultData.error || 0) }}</span>条，导入失败<span class="result-info__detail--err">{{ resultData.error || 0 }}</span>条</p>
          <el-button
...
export default {
  ...
  methods: {
    ...
    updateFile(result) {
      if (!this.file.name) {
        ...
        userExcelImportAPI({
          ...
          cover: 0,
          config: 1,
          owner_user_id: this.userInfo.id,
          file: this.file
        })
          .then(res => {
            this.loading = false
     ...
     */
    downloadErrData() {
      this.getImportError(this.resultData)
    },

    /**
     * 导入错误下载
     */
    getImportError(resultData) {
      this.loading = true
      userErrorExcelDownAPI({
        name: '导入错误数据',
        path: resultData.error_file_path
      })
        .then(res => {
    ...
    download() {
      ...
      this.loading = true
      userImportTemplateAPI().then(res => {
        downloadExcelWithResData(res)
        this.loading = false
      }).catch(() => {
        this.loading = false
