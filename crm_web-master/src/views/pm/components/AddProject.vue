        ...
        <div class="cover">
          ...
          <flexbox
            ...
            <flexbox-item
              ...
              <img v-src="item.url || item.path" class="cover-img">
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      ...
      workWorkReadAPI({
        ...
        .then(res => {
          ...
          this.openType = data.is_open
          this.ownerRole = data.group_id
          if (this.openType == 0) {
          ...
          if (data.cover_url) {
            this.coverImg = {
              custom: data.is_system_cover != 1,
              url: data.cover_url
            }
            if (data.is_system_cover != 1) {
    ...
    submitBtn() {
      ...
      if (this.openType == 0) {
        params.owner_user_id = this.selectUserList
        ...
        params.group_id = this.ownerRole
      }
      ...
      request(params)
        .then(res => {
          ...
          const work_id = res.data ? res.data.work_id : ''
          if (this.enterDetail && work_id) {
            this.$bus.emit('add-project', this.name, work_id)
            this.close()
            this.$router.push({
              name: 'project-list',
              params: {
