          ...
          <draggable
            ...
            <template v-if="tabShowType == 'board'">
              <div
                ...
                <div v-src:background-image="childItem.cover_url || defaultCorverUrl" class="cover-content-item__content">
                  <div class="handle-bar">
                    ...
                    <i
                      :class="{ 'is-collect': childItem.is_follow == 1 }"
...
export default {
  ...
  data() {
    return {
      ...
      isCreate: false,
      list: [],
      defaultCorverUrl: '/static/pc/images/pm/project-cover-1.jpg',
      tabName: 'all',
      tabList: [
        {
          label: '全部项目',
  ...
  methods: {
     ...
     */
    collectClick(item) {
      workWorkCollectAPI({ work_id: item.work_id, follow: item.is_follow ? 0 : 1 })
        .then(res => {
    ...
    moveItem(evt) {
      if (evt) {
        ...
        workupdateWorkOrderAPI({
          workIds: this.tabList[0].list.map(item => item.work_id)
        })
          .then(res => {})
          .catch(() => {})
      }
    ...
    deleteProject(data) {
      this.$confirm(
        ...
        .then(() => {
          workWorkDeleteAPI({ work_id: data.work_id })
            .then(res => {
              this.$message({
                type: 'success',
                message: '删除成功!'
