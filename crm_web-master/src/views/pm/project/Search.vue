  ...
  methods: {
    ...
    refreshList() {
      ...
      if (this.form.type != 'all') {
        ...
        if (this.form.type == 5) {
          params.start_time = this.form.startTime
          params.end_time = this.form.endTime
        }
      }


      if (this.form.userList && this.form.userList.length) {
        params.owner_user_id = this.form.userList
      }
    ...
    getTaskList(params) {
      ...
      workQueryWorkTaskListAPI({
        ...params,
        sort_type: this.form.sort,
        work_id: this.form.workIdList,
        label_id: this.form.labelIdList
      })
        .then(res => {
          const list = res.data || []
          list.forEach(item => {
    ...
    projectCellHandle(type, data) {
      if (type === 'edit') {
        this.editId = data.work_id
        this.isCreate = true
      } else if (type === 'delete') {
        this.deleteProject(data)
      } else {
        workWorkCollectAPI(data.work_id)
          .then(res => {
            this.refreshList()
          })
          .catch(() => {})
