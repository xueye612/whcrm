...
import {
  ...
  workWorkAddUserSetRoleGroupAPI
} from '@/api/pm/project'
import {
...
export default {
  ...
  methods: {
    ...
    userSelectChange(members, dep) {
      // const selectIds = this.membersList.map(item => item.id)
      ...
      //   if (memder) {
      //     item.id = memder.id
      //     item.title = memder.title
      //   } else {
      //     item.id = roleItem ? roleItem.id : ''
      //     item.title = roleItem ? roleItem.title : ''
      //   }
    ...
    getSelectMemberRole(selectIds, member) {
      if (selectIds.includes(member.id)) {
        return this.membersList.find(item => item.id)
      }
