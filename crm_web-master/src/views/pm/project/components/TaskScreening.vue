      ...
      <div class="content">
        ...
        <div
          ...
          <div
            ...
            <span>{{ val.name }}</span> -->
            <span :class="{ 'is-tag': val.type == 'tag'}" class="item-list-label">{{ val.name ||val.realname }}</span>
    ...
    getUserList() {
      ...
      request(params).then(res => {
        this.menuList[0].list = res.data.map(item => {
          item.checked = this.data && this.data.userIds && this.data.userIds.includes(item.id)
          item.name = item.name
          item.type = 'user'
          return item
        })
      ...
      workTasklableIndexAPI()
        .then(res => {
          this.menuList[2].list = res.data.list.map(item => {
            ...
            item.checked = this.data && this.data.tagIds && this.data.tagIds.includes(item.lable_id)
            item.type = 'tag'
            return item
          })
        })
