  ...
  methods: {
    ...
    moveEndParentTask(evt) {
      ...
      if (evt && evt.oldIndex != evt.newIndex) {
        workTaskUpdateClassOrderAPI({
          work_id: this.workId,
          class_ids: this.taskList.map(item => item.class_id)
        })
          .then(res => {})
          .catch(() => {})
      }
    ...
    moveEndSonTask(evt) {
      ...
      if (evt) {
        ...
        if (fromId == toId) {
          params = {
            toList: toList.map(item => item.task_id),
            toId: toId,
            work_id: this.workId
          }
        } else {
          params = {
            fromlist: fromList.map(item => item.task_id),
            fromid: fromId,
            tolist: toList.map(item => item.task_id),
            toid: toId,
            work_id: this.workId
     ...
     * 勾选
     */
    async checkboxChange(element, value, fromIndex) {
    ...
    changeListCompleteOrder(element, value, fromIndex, upload = true) {
      ...
      if (toIndex != null && toIndex >= 0) {
        if (toIndex != fromIndex) {
          ...
          if (upload) {
            workTaskUpdateOrderAPI({
              toList: value.list.map(item => item.task_id),
              toId: value.class_id,
              work_id: this.workId
            })
              .then(res => {})
     ...
     * 创建新任务
     */
    createSubTaskClick(val, index) {
      this.$refs.taskQuickAdd[index].moreClick()
    ...
    detailHandle(data) {
      if (data.index == 0 || data.index) {
        ...
        if (data.type == 'title-check') {
          ...
          this.$set(this.taskList[data.section].list[data.index], 'lableList', data.value)
        }
      }
    },
