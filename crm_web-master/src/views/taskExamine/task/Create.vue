            ...
            <template v-if="fieldForm[data.field]">
              <div
                ...
                <flexbox
                  ...
                  <div
                    v-if="item.stopTime ||item.stop_time"
                    style="margin-left: 8px;"
                    class="bg-color task-bg-color">{{ ( item.stopTime||item.stop_time) | moment("YYYY-MM-DD") }} 截止</div>
                  <xr-avatar
                    v-if="item.mainUser"
...
export default {
  ...
  created() {
    const fieldForm = {
      priority: 0,
      main_user_id: this.userInfo.id,
      owner_user_id: [this.userInfo]
    }
    ...
    if (this.action.data) {
      ...
      if (actionData.main_user_id) {
        fieldForm.main_user_id = actionData.main_user_id
      }
      ...
      this.fieldList.forEach(item => {
        if (item.form_type === 'user' && item.radio == false) {
          ...
          params['customer_ids'] = value.customerIds && value.customerIds.length
            ? value.customerIds
            : []
          params['contacts_ids'] = value.contactsIds && value.contactsIds.length
            ? value.contactsIds
            : []
          params['business_ids'] = value.businessIds && value.businessIds.length
            ? value.businessIds
            : []
          ...
          params[item.field] = valueList.map(item => {
            return {
              ...
              owner_user_id: item.mainUser ? item.mainUser.id : '',
              name: item.name,
