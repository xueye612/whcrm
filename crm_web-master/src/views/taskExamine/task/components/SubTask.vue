  ...
  created() {
    if (this.subTaskCom == 'edit') {
      ...
      if (JSON.stringify(this.subData) !== '{}') {
        if (this.subData.mainUser || this.subData.main_user_id) {
          this.xhUserData = this.subData.mainUser ? [this.subData.mainUser] : [this.subData]
        }
      } else {
        this.xhUserData = []
      }
          ...
          // this.$emit('on-handle', { type: 'add', result: 'success' })
            ...
            // this.$emit('on-handle', { type: 'edit', result: 'success' })
              workSubTaskUpdateAPI({
                ...
                stop_time: this.subtasksDate,
                main_user_id:
                this.xhUserData.length > 0 ? this.xhUserData[0].id || this.xhUserData[0].main_user_id : '',
                name: this.subtasksTextarea
              })
                .then(res => {
