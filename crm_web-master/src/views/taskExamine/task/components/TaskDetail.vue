    ...
    <flexbox
      ...
      <div
        ...
        <flexbox
          ...
          <span
            v-if="taskData.create_user_info"
            class="task-hd__top--create">{{ taskData.create_user_info.realname }} 创建于 {{ taskData.create_time }}</span>
          <div>
            <span
              v-if="isArchive"
              class="task-hd__top--info">该任务已于 {{ taskData.archive_time }} 被归档</span>
            <el-button
              v-if="isArchive"
            ...
            <span
              v-if="isRecycle"
              class="task-hd__top--info">该任务已于 {{ taskData.hidden_time }} 被放入回收站</span>
            <el-button
        ...
        <flexbox class="task-hd__bottom">
          ...
          <flexbox-item>
            <xh-user-cell
              :value="taskData.main_user ? [taskData.main_user] : []"
              ...
              <flexbox
                ...
                class="head-btn">
                <i
                  v-if="!taskData.main_user"
                  class="wk wk-l-plus head-btn__icon is-null" />
                <xr-avatar
                  v-else
                  :name="taskData.main_user ? taskData.main_user.realname : ''"
                  :size="32"
                  :src="taskData.main_user ? taskData.main_user.img : ''"
                  class="user-img" />
                <div class="head-btn__bd">
                  <div
                    v-if="taskData.main_user"
                    class="head-btn__bd--title">{{ taskData.main_user.realname }}</div>
                  <div class="head-btn__bd--des">负责人</div>
                </div>
                <i
                  v-show="taskData.main_user && getPermission('setTaskMainUser')"
          ...
          <flexbox-item>
            <flexbox class="head-btn">
              <i
                :class="[ taskData.start_time ? 'is-valve' : 'is-null']"
              ...
              <el-date-picker
                ...
                @change="timeChange('start_time')" />
              <div class="head-btn__bd">
                <div
                  v-if="taskData.start_time"
                  class="head-btn__bd--title">{{ taskData.start_time | moment('YYYY年MM月DD日') }}</div>
                <div class="head-btn__bd--des">开始时间</div>
              </div>
              <i
                v-show="taskData.start_time"
                class="el-icon-close head-btn__close"
                @click="deleteTime('start_time')" />
            </flexbox>
          </flexbox-item>
          <flexbox-item>
            <flexbox class="head-btn">
              <i
                :class="[ taskData.stop_time ? 'is-valve' : 'is-null']"
              ...
              <el-date-picker
                ...
                type="date"
                value-format="yyyy-MM-dd"
                @change="timeChange('stop_time')" />
              <div class="head-btn__bd">
                <div
                  v-if="taskData.stop_time"
                  class="head-btn__bd--title">{{ taskData.stop_time | moment('YYYY年MM月DD日') }}</div>
                <div class="head-btn__bd--des">结束时间</div>
              </div>
              <i
                v-show="taskData.stop_time"
                class="el-icon-close head-btn__close"
                @click="deleteTime('stop_time')" />
            </flexbox>
          </flexbox-item>
        </flexbox>
      </div>
          ...
          <flexbox>
            <flexbox-item class="participant">
              ...
              <flexbox
                ...
                wrap="wrap">
                <span
                  v-for="(item, index) in taskData.owner_list"
                  :key="index"
                  class="owner-list">
                  <el-tooltip
                    placement="bottom"
                ...
                <members-dep
                  ...
                  :close-dep="true"
                  :content-block="false"
                  :user-checked-data="taskData.owner_list"
          ...
          <div class="secitons">
            ...
            <div class="section">
              <div class="section__hd">
                ...
                <span>子任务({{ subTaskComplete }}/{{ taskData.subTaskList.length }})</span>
              </div>
              <div class="section__bd">
                    ...
                    <div
                      ...
                      <flexbox
                        ...
                        <div
                          ...
                          class="bg-color task-bg-color">{{ item.stop_time | moment("YYYY-MM-DD") }} 截止</div>
                        <xr-avatar
                          v-if="item.mainUser||item.main_user_id"
                          :name="(item.mainUser && item.mainUser.realname)|| item.realname"
                          :size="25"
                          :src="(item.mainUser &&item.mainUser.thumb_img)||item.thumb_img"
                          class="user-img" />
                ...
                <flexbox
                  ...
                  <div class="activity-cell__bd">
                    <div class="activity-info">
                      ...
                      <span class="activity-info--time">{{ item.create_time }}</span>
                    </div>
...
export default {
  ...
  data() {
     ...
     */
      startTimeOptions: {
        disabledDate: (time) => {
          if (!this.taskData || !this.taskData.stop_time) {
            return false
          }
          return moment(time).isAfter(this.taskData.stop_time)
        }
      },
      stopTimeOptions: {
        disabledDate: (time) => {
          if (!this.taskData || !this.taskData.start_time) {
            return false
          }
          return moment(time).isBefore(this.taskData.start_time)
        }
      },
      // 是否显示子任务
      addSubtasks: true,
  ...
  computed: {
     ...
     */
    isArchive() {
      return this.taskData.is_archive == 1 && this.taskData.ishidden != 1
    },
     ...
     */
    subTaskProgress() {
      if (this.subTaskComplete == 0) {
        return 0
      }
      return parseInt(
        (this.subTaskComplete / this.taskData.subTaskList.length) * 100
      )
    },
    ...
    ownerListParams() {
      return this.workId ? { work_id: this.workId } : null
    },
     ...
     */
    workId() {
      return this.taskData && this.taskData.work_id
    },
  ...
  methods: {
      ...
      request({ task_id: this.id })
        .then(res => {
          ...
          if (taskData.subTaskList) {
            for (const item of taskData.subTaskList) {
              if (item.status == 5) {
                item.checked = true
          ...
          this.fileList = taskData.fileList || []
     ...
     * 完成任务
     */
    async completeMainTask(val) {
      this.taskData.checked = val
      const status = this.taskData.checked ? 5 : 1
    ...
    getPermission(key) {
      ...
      const permission = this.taskData ? this.taskData.auth || {} : {}
      return !!permission[key]
    },

    handleLabelChange(labelList) {
      const list = labelList || []
      this.$set(this.taskData, 'lable_list', list)
      this.$emit('on-handle', {
        type: 'change-label',
        value: list,
        index: this.detailIndex,
    ...
    moreDelete() {
      this.$confirm('确定删除?', '提示', {
        ...
        .then(() => {
          workTaskDeleteAPI({ task_id: this.id, work_id: this.workId })
            .then(res => {
    ...
    moreArchive() {
      this.$confirm('此操作将归档该任务, 是否继续?', '提示', {
        ...
        .then(() => {
          workTaskArchiveAPI({ task_id: this.id, work_id: this.workId })
            .then(res => {
    ...
    subtasksCheckbox(val, e) {
      ...
      //   value: {
      //     subdonecount: this.subTaskComplete,
     ...
     */
    mainUserChange(data) {
      const main_user = data.value.length > 0 ? data.value[0] : null
      this.submiteMainUser(main_user)
    },

    /**
     * 上传负责人信息
     */
    submiteMainUser(main_user) {
      workTaskMainUserSetAPI({
        work_id: this.workId,
        task_id: this.id,
        main_user_id: main_user ? main_user.id : '',
        type: 'main_user_id'
      })
        .then(res => {
          if (main_user) {
            this.$set(this.taskData, 'main_user', main_user)
          } else {
            this.$set(this.taskData, 'main_user', null)
          }
    ...
    deleteSubTask(val) {
      this.$confirm('此操作将删除该任务, 是否继续?', '提示', {
        ...
        .then(() => {
          workSubTaskDeleteAPI({ task_id: val.task_id, work_id: this.workId })
            .then(res => {
              const subData = this.taskData.subTaskList
    ...
    editSubTask(val) {
      ...
      const dataList = this.taskData.subTaskList
    ...
    activateTask() {
      this.loading = true
      workTaskRecoverAPI({ task_id: this.taskData.task_id, work_id: this.workId })
        .then(res => {
          this.loading = false
          this.$emit('on-handle', {
            type: 'activate-task',
    ...
    recoverTask() {
      this.loading = true
      workTrashRecoverAPI({ task_id: this.taskData.task_id, work_id: this.workId })
        .then(res => {
          this.loading = false
    ...
    thoroughDeleteTask() {
      this.$confirm('确定删除?', '提示', {
        ...
        .then(() => {
          this.loading = true
          workTrashDeleteAPI({ task_id: this.taskData.task_id, work_id: this.workId })
            .then(res => {
              this.loading = false
              this.$emit('on-handle', {
