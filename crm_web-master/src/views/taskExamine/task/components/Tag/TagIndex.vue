...
export default {
  ...
  methods: {
    ...
    updateTaskLabels() {
      const labelList = this.getCheckedLabelList()
      this.taskData.lable_list = labelList
      this.$emit('update-label', labelList)
    },
    /**
     * 创建新标签
     */
      ...
      if (this.$listeners.change) {
        ...
        if (value.check) {
          workTaskLabelSetAPI({
            ...
            if (err && err.code === null && err.error === null) {
              value.check = false
              this.updateTaskLabels()
              return
            }
    ...
    updateDetailList(value, type) {
      if (this.taskData && this.taskData.lable_list) {
        let changeIndex = -1
        for (let index = 0; index < this.taskData.lable_list.length; index++) {
          const element = this.taskData.lable_list[index]
          if (element.lable_id == value.lable_id) {
            changeIndex = index
            break
          }
        }
        if (changeIndex >= 0) {
          if (type == 'delete') {
            this.taskData.lable_list.splice(changeIndex, 1)
          } else {
            this.taskData.lable_list.splice(changeIndex, 1, value)
          }
        }
      }
    },
      ...
      if (this.newTagTitle == '创建新标签') {
        ...
        workTasklableSetAPI({
          ...
          for (const item of this.tagList) {
            if (item.lable_id == this.editTagId) {
              item.name = val
              item.color = color
              item.labelName = item.name
        ...
        .then(() => {
          workTasklableDeleteAPI({
            ...
            for (const i in this.tagList) {
              if (this.tagList[i].lable_id == val.lable_id) {
    ...
    getTagList() {
      ...
      workTasklableIndexAPI().then(res => {
        ...
        const selectLabels = this.taskData && this.taskData.lable_list || []
        const selectIds = selectLabels.map(item => item.lable_id)
        for (const item of dataList) {
          item.check = selectIds.includes(item.lable_id)
        }
        // 标签管理数据
