    ...
    <div class="dialog-body">
      ...
      <div
        ...
        <div class="result-info">
          ...
          <p v-if="resultData" class="result-info__detail">导入总数据<span class="result-info__detail--all">{{ resultData.total }}</span>条，<template v-if="resultData.cover">覆盖<span class="result-info__detail--suc">{{ resultData.done }}</span>条，</template>导入成功<span class="result-info__detail--suc">{{ resultData.total - resultData.error }}</span>条，导入失败<span class="result-info__detail--err">{{ resultData.error }}</span>条</p>
          <el-button
...
export default {
  ...
  computed: {
    ...
    fieldUniqueInfo() {
      const uniqueList = this.fieldList.filter(item => item.is_unique == 1)
  ...
  methods: {
    ...
    downloadErrData() {
      this.loading = true
      crmDownImportErrorAPI({ name: '导入错误数据', path: this.resultData.error_file_path })
        .then(res => {
