<template>
  ...
  <el-input
    ...
    :value="fieldFrom[item.field] && fieldFrom[item.field].address ? fieldFrom[item.field].address : ''"
    :disabled="item.disabled || disabled"
    :placeholder="item.placeholder || '地址'"
    @input="locationChange(item, index, $event)"/>
  <wk-signature-pad
...
export default {
  ...
  methods: {
    locationChange(item, index, value) {
      const current = this.fieldFrom[item.field]
      const next = current && typeof current === 'object'
        ? { ...current, address: value }
        : { lat: '', lng: '', address: value }
      this.$set(this.fieldFrom, item.field, next)
      this.commonChange(item, index, next)
    }
  }
}
</script>
