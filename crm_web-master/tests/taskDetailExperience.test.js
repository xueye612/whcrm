...
check(cellVue.includes('class="more-count"'), 'multiple task labels use a compact count marker')
check(cellVue.includes("'has-more-labels': hideShowLabels.length"), 'multiple labels use constrained layout styling')
check(!cellVue.includes('class="more-btn"'), 'multiple labels no longer use the overflowing ellipsis button')
