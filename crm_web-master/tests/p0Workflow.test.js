...
// ===================== 菜单名称与图标修改 =====================
var pmRouteSrc = fs.readFileSync(path.resolve(__dirname, '../src/router/modules/pm.js'), 'utf8')
assert.ok(pmRouteSrc.indexOf('奖惩') !== -1, '项目菜单应展示"奖惩"')
assert.ok(pmRouteSrc.indexOf('绩效') !== -1, '项目菜单应展示"绩效"')
assert.ok(pmRouteSrc.indexOf("icon: 'business-intelligence'") !== -1, '绩效菜单应使用可正常显示的分析图标')
assert.ok(pmRouteSrc.indexOf("icon: 'results'") === -1, '不应再使用 results 图标（不存在）')
assert.ok(pmRouteSrc.indexOf('奖励候选') === -1, '不应再有"奖励候选"')
assert.ok(pmRouteSrc.indexOf('季度绩效') === -1, '不应再有"季度绩效"')
// 路由 path 和权限编码不变
assert.ok(pmRouteSrc.indexOf("path: 'reward'") !== -1, '奖惩子路由 path 不变')
assert.ok(pmRouteSrc.indexOf("path: 'performance'") !== -1, '绩效子路由 path 不变')
assert.ok(pmRouteSrc.indexOf("'crm', 'reward', 'candidatelist'") !== -1, '奖惩权限编码不变')
assert.ok(pmRouteSrc.indexOf("'crm', 'performance', 'summarylist'") !== -1, '绩效权限编码不变')
