...
class Event extends Common
{
    ...
    public function eventCrm($param)
    {
        ...
        $between_time = '';
        //闇€瑕佽仈绯荤殑瀹㈡埛 next_time crm_customer
        ...
        $event_date = db('crm_customer')->where(['next_time' => ['>', 0], 'owner_user_id' => ['<>', 0]])->where($between_time)->where('')
            ->field('next_time as start_time')->group('next_time')->select();
        $item = [];
        ...
        $event_date = db('crm_business')->where($between_time)->where('next_time', '>', 0)->field('next_time as start_time')->group('next_time')->select();
        unset($item);
        ...
        // 瀹㈡埛鍙拌处锛堜紭鍏?finish_time锛屽叾娆?register_time锛?
        $ledgerList = db('customer_ledger')
            ->where(function ($query) use ($start_time, $end_time) {
                $query->where(function ($q) use ($start_time, $end_time) {
                    $q->where('finish_time', '>', 0)
                        ->where('finish_time', 'between', [$start_time, $end_time]);
                })->whereOr(function ($q) use ($start_time, $end_time) {
                    $q->where('finish_time', 0)
                        ->where('register_time', 'between', [$start_time, $end_time]);
                });
            })
            ->where(function ($query) use ($user_id) {
                $query->where('register_user_id', $user_id)
                    ->whereOr('handler_user_id', $user_id);
            })
            ->field('ledger_id,title,finish_time,register_time')
            ->select();
        $ledgerItems = [];
        foreach ($ledgerList as $row) {
            $eventTime = $row['finish_time'] ? $row['finish_time'] : $row['register_time'];
            $ledgerItems[] = [
                'ledger_id' => $row['ledger_id'],
                'title' => $row['title'],
                'event_time' => $eventTime ? date('Y-m-d H:i:s', $eventTime) : null
            ];
        }
        $list_data['ledger_list'] = $ledgerItems;
        return $list_data ?: [];
    }
    ...
    {
        // Ensure system type: 台账
        $ledgerSchedule = db('admin_oa_schedule')->where(['name' => '台账', 'type' => 1])->find();
        if (!$ledgerSchedule) {
            db('admin_oa_schedule')->insert([
                'name' => '台账',
                'color' => '8',
                'type' => 1,
                'create_time' => time(),
...
//            actionLog($eventId, $param['owner_user_ids'], '', '创建了日程');
    ...
    public function eventBusiness($param)
    {
        ...
        $return_time = `'` . strtotime(date('Y-m-d', $time[0])) . `'`;
        $return_end = `'` . strtotime(date('Y-m-d 23:59:59', $time[1])) . `'`;
        //需要联系的商机 next_time crm_customer
