...
class Common extends Model
{
    ...
    public function fmtRequest($request = [])
    {
        ...
        \think\Log::write('Common fmtRequest - 接收到的原始请求: ' . json_encode($request, JSON_UNESCAPED_UNICODE), 'info');
        $pageType = !empty($request['pageType']) ? 'all' : ''; //all全部（不分页）
        $page = 1;
        ...
        if ($pageType == 'all') {
            ...
            // 大数据处理-滚动加载-子分页
            if (isset($request['sub_page']) && $request['sub_page'] && isset($request['sub_limit']) && $request['sub_limit']) {
        ...
        // 检查原始请求中是否存在 'map' 键，如果存在，则使用它作为筛选条件
        // 否则，将剩余的 $request 数组作为筛选条件
        $map = isset($request['map']) ? $request['map'] : $request;
        ...
        // 调试：记录 fmtRequest 返回的 ret 结构
        \think\Log::write('Common fmtRequest - 返回的ret结构: ' . json_encode($ret, JSON_UNESCAPED_UNICODE), 'info');
        return $ret;
    }
