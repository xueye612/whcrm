...
class CustomerLedger extends Common
{
    ...
    public function getDataList($request, $userId)
    {
        ...
        $query = Db::name($this->name)
            ...
            ->join('__TASK_WORKFLOW__ task_workflow', 'ledger.task_id = task_workflow.task_id', 'LEFT')
            ->field("ledger.*,task_workflow.main_status as task_main_status,task_workflow.init_w as task_init_w,task_workflow.init_r as task_init_r,task_workflow.init_k as task_init_k,customer.name as customer_name,customer.crm_qpmlfv as customer_short_name,register_user.realname as register_user_name,handler_user.realname as handler_user_name,business.name as business_name,contract.num as contract_num,contract.name as contract_name,contract.crm_defqwa as contract_short_name,contract.end_time as contract_end_time,(SELECT record.content FROM {$recordTable} record WHERE record.ledger_id = ledger.ledger_id AND record.new_status = '已完成' AND record.content <> '创建台账' AND record.content NOT LIKE '状态变更：%' AND record.content NOT LIKE '同步任务：%' ORDER BY record.create_time DESC,record.record_id DESC LIMIT 1) as completed_reply,(SELECT record.content FROM {$recordTable} record WHERE record.ledger_id = ledger.ledger_id AND record.new_status = '已关闭' AND record.content <> '创建台账' AND record.content NOT LIKE '状态变更：%' AND record.content NOT LIKE '同步任务：%' ORDER BY record.create_time DESC,record.record_id DESC LIMIT 1) as closed_reason,(SELECT COUNT(1) FROM {$recordTable} record WHERE record.ledger_id = ledger.ledger_id) as record_count")
            ->where($where);
        if ($keywordWhere) {
    ...
    public function getAccessibleById($ledgerId, $userId)
    {
        ...
        $query = Db::name($this->name)
            ...
            ->join('__CRM_CONTRACT__ contract', 'ledger.contract_id = contract.contract_id', 'LEFT')
            ->join('__TASK_WORKFLOW__ task_workflow', 'ledger.task_id = task_workflow.task_id', 'LEFT')
            ->field("ledger.*,task_workflow.main_status as task_main_status,task_workflow.init_w as task_init_w,task_workflow.init_r as task_init_r,task_workflow.init_k as task_init_k,customer.name as customer_name,customer.crm_qpmlfv as customer_short_name,register_user.realname as register_user_name,handler_user.realname as handler_user_name,business.name as business_name,contract.num as contract_num,contract.name as contract_name,contract.crm_defqwa as contract_short_name,contract.end_time as contract_end_time,(SELECT record.content FROM {$recordTable} record WHERE record.ledger_id = ledger.ledger_id AND record.new_status = '已完成' AND record.content <> '创建台账' AND record.content NOT LIKE '状态变更：%' AND record.content NOT LIKE '同步任务：%' ORDER BY record.create_time DESC,record.record_id DESC LIMIT 1) as completed_reply,(SELECT record.content FROM {$recordTable} record WHERE record.ledger_id = ledger.ledger_id AND record.new_status = '已关闭' AND record.content <> '创建台账' AND record.content NOT LIKE '状态变更：%' AND record.content NOT LIKE '同步任务：%' ORDER BY record.create_time DESC,record.record_id DESC LIMIT 1) as closed_reason")
