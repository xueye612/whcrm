...
class Task extends Common
{
    ...
    public function getDataList($request, $user_id)
    {
        ...
        $work_id = isset($request['work_id']) ? $request['work_id'] : 0;
        $stopTimeType = isset($request['stop_time_type']) ? (string)$request['stop_time_type'] : '';
        $labelIds = !empty($request['lable_id']) && is_array($request['lable_id']) ? $request['lable_id'] : [];
        $mainUserIds = !empty($request['main_user_id']) ? $request['main_user_id'] : [];
        ...
        $classRows = !empty($classList['list']) ? $classList['list'] : [];
        if ($classArr && $classRows) {
            $newList = array_merge($classArr, $classRows);
        } elseif ($classArr) {
            $newList = $classArr;
        } else {
            $newList = $classRows;
        ...
        if ($stopTimeType !== '') {
            if ($stopTimeType == '5') { //没有截至日期
                $map['stop_time'] = '0';
            } elseif ($stopTimeType == '6') { //延期的
                $map['stop_time'] = ['between', [1, time()]];
                $map['status'] = 1;
            } elseif ($stopTimeType == '7') { //今日更新
    ...
    public function getOwnerTaskList($request, $user_id)
    {
        ...
        $stopTimeType = isset($request['stop_time_type']) ? (string)$request['stop_time_type'] : '';
        $labelIds = !empty($request['lable_id']) && is_array($request['lable_id']) ? $request['lable_id'] : [];
        $filterMainUserIds = !empty($request['main_user_id']) ? $request['main_user_id'] : [];
        $ret = $workModel->checkWork($work_id, $user_id);
        if (!$ret) {
            $this->error = $workModel->getError();
            return false;
        ...
        if ($filterMainUserIds) {
            $map['main_user_id'] = ['in', $filterMainUserIds];
        }
        //截止时间
        if ($stopTimeType !== '') {
            if ($stopTimeType == '5') { //没有截至日期
    ...
    public function updateDetTask($param)
    {
        ...
        $target_name=$taskInfo['name'];
        switch ($type) {
            case 'name' :
                $data['after'] = $param['name'];
                $name='任务名称';
                $count_name=$param['name'];
                break;
            case 'stop_time' :
                if ($param['stop_time']) {
            ...
            case 'lable_id_add' :
                //标签添加
                $oldLabel = !empty($taskInfo['lable_id']) ? $lableModel->getNameByIds(stringToArray($taskInfo['lable_id'])) : [];
                ...
                $addLabel = !empty($lable) ? array_diff($lable, $oldLabel) : '';
                $data['after'] = !empty($addLabel) ? implode(',', $addLabel) : '';
        ...
        if ($flag || count($rdata)) {
            ...
            if ($notifyEvent) {
                (new DingTalkLogic())->sendTaskNotify($notifyEvent, $param['task_id'], $createUserId, [
                    'summary' => '任务负责人变更'
                ]);
            }
            return (int)$param['task_id'];
        } else {
            $this->error = '操作失败';
            return 0;
        }
    ...
    public function archiveData($param)
    {
        ...
        $dataInfo=$this->get($param['task_id']);
    ...
    public function getTaskList($request)
    {
        ...
        $search = isset($request['search']) ? $request['search'] : '';
        $whereStr = isset($request['whereStr']) ? $request['whereStr'] : [];
        $lable_id = isset($request['lable_id']) ? $request['lable_id'] : '';
        $main_user_id = isset($request['main_user_id']) ? $request['main_user_id'] : [];
        ...
        $lableModel = new \app\work\model\WorkLable();
        $map = $requestMap;
        $map['ishidden'] = isset($requestMap['ishidden']) ? $requestMap['ishidden'] : 0;
