...
class Leads extends Common
{
    ...
    public function getDataList($request)
    {
        ...
        $overdue = $request['overdue'] ?? 0; // 待办事项下需联系线索(逾期)
        if (isset($request['scene_id'])) unset($request['scene_id']);
        if (isset($request['search'])) unset($request['search']);
        if (isset($request['user_id'])) unset($request['user_id']);
        if (isset($request['is_excel'])) unset($request['is_excel']);
        if (isset($request['order_field'])) unset($request['order_field']);
        if (isset($request['order_type'])) unset($request['order_type']);
        if (isset($request['getCount'])) unset($request['getCount']);
        if (isset($request['overdue'])) unset($request['overdue']);
...
//        $map = where_arr($map, 'crm', 'leads', 'index');
        ...
        // Filter invalid fields to avoid selecting non-existent columns
        $tableFields = Db::getTableFields(config('database.prefix') . 'crm_leads');
        if (!empty($tableFields)) {
            $filtered = [];
            foreach ($indexField as $field) {
                $baseField = strpos($field, '.') !== false ? substr($field, strrpos($field, '.') + 1) : $field;
                if (in_array($baseField, $tableFields)) {
                    $filtered[] = $field;
                }
            }
            $indexField = $filtered;
        }
        if (!in_array('leads_id', $indexField) && !in_array('leads.leads_id', $indexField)) {
            $indexField[] = 'leads.leads_id';
        }
        ...
        //过滤已转化线索
        if (!isset($map['leads.is_transform']) || !$map['leads.is_transform']) {
