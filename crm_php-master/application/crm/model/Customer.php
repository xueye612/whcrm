...
class Customer extends Common
{
	...
	public function getDataList($request)
    {
    	...
    	$is_remind   = $request['is_remind'] ?? 0;
    	$getCount    = $request['getCount'] ?? 0;
    	$otherMap    = $request['otherMap'] ?? [];
        $overdue = $request['overdue'] ?? 0; // 待办事项下需联系客户(逾期)
        $is_ledger_filter = $request['is_ledger_filter'] ?? 0;
        $is_finance_filter = $request['is_finance_filter'] ?? 0;
    	# 需要过滤的参数
    	$unsetRequest = ['scene_id','search','user_id','is_excel','action','order_field','order_type','is_remind','getCount','type','otherMap','poolId','overdue','is_ledger_filter','is_finance_filter'];
        ...
        $sceneMap = [];
        if (empty($getCount) && empty($is_ledger_filter) && empty($is_finance_filter)) {
            if ($scene_id) {
                //自定义场景
		...
		if ($is_ledger_filter) {
			...
			$ledgerWhere = "EXISTS (SELECT 1 FROM {$ledgerTableName} WHERE customer_id = customer.customer_id)";
		}
		$financeWhere = [];
		if ($is_finance_filter) {
			$financeTableName = Db::name('finance_record')->getTable();
			$financeWhere = "EXISTS (SELECT 1 FROM {$financeTableName} WHERE customer_id = customer.customer_id)";
		}
		...
		if ($order_type && $order_field && $order_field != 'owner_user_structure_name') {
			if ($order_field == 'ledger_count') {
				$ledgerTableName = Db::name('customer_ledger')->getTable();
				$order = "(SELECT COUNT(*) FROM {$ledgerTableName} WHERE customer_id = customer.customer_id) {$order_type}, customer.create_time asc";
			} else {
        ...
        if (!empty($list)) {
			...
			$ledger_count = Db::name('customer_ledger')
				->field([
					'COUNT(*)' => 'count',
					'customer_id'
				])
				...
				->group('customer_id')
				->select();
			$ledger_count = array_column($ledger_count, null, 'customer_id');

			$contract_count = Db::name('crm_contract')
				->field([
					'COUNT(*)' => 'count',
					'customer_id'
				])
				...
				->group('customer_id')
				->select();
			$contract_count = array_column($contract_count, null, 'customer_id');
			...
			foreach ($list AS $k => $v) {
				...
				# 合同数
				$list[$k]['contract_count'] = $contract_count[$v['customer_id']]['count'] ?: 0;
				# 台账数
				$list[$k]['ledger_count'] = $ledger_count[$v['customer_id']]['count'] ?: 0;
	        	# 权限
