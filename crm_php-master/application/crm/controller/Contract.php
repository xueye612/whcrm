...
class Contract extends ApiCommon
{
    ...
    **/    
    public function _initialize()
    {
        $action = [
            'permission'=>[''],
            'allow'=>['check', 'revokecheck', 'product', 'system', 'count', 'copy']
        ];
    ...
    public function index()
    {
        ...
        if ($isLedgerFilter && !empty($data['list']) && is_array($data['list'])) {
            $contractIds = array_values(array_unique(array_filter(array_map(function ($item) {
                return (int)($item['contract_id'] ?? 0);
            }, $data['list']))));
            $ledgerCountMap = [];
            if (!empty($contractIds)) {
                $rows = Db::name('customer_ledger')
                    ->field('contract_id,count(*) as cnt')
                    ->where('contract_id', 'in', $contractIds)
                    ->group('contract_id')
                    ->select();
                foreach ($rows as $row) {
                    $ledgerCountMap[(int)$row['contract_id']] = (int)$row['cnt'];
                }
            }
            foreach ($data['list'] as &$item) {
                $cid = (int)($item['contract_id'] ?? 0);
                $item['ledger_count'] = (int)($ledgerCountMap[$cid] ?? 0);
            }
            usort($data['list'], function ($a, $b) {
                $c1 = (int)($a['ledger_count'] ?? 0);
                $c2 = (int)($b['ledger_count'] ?? 0);
                if ($c1 === $c2) {
                    return ((int)($b['contract_id'] ?? 0)) - ((int)($a['contract_id'] ?? 0));
                }
                return $c2 - $c1;
            });
        }
    ...
    public function save()
    {
        ...
        $numberInfo = [];
        if (empty($param['num'])) {
            $numberInfo = $this->getAutoNumbers(1);
    ...
    public function read()
    {
        ...
        $teamOnly = !empty($param['team_only']);
        //判断权限
        ...
        if ($teamOnly) {
            if ($data['owner_user_id'] != $userInfo['id'] && !$roPre && !$rwPre) {
                $authData['dataAuth'] = (int)0;
                return resultArray(['data' => $authData]);
            }
        } elseif (!in_array($data['owner_user_id'],$auth_user_ids) && !$roPre && !$rwPre) {
            $authData['dataAuth'] = (int)0;
            return resultArray(['data' => $authData]);
        }
        if (!$data) {
    ...
    public function delete()
    {
        ...
        foreach ($contract_id as $k=>$v) {
            ...
            if (!in_array($data['owner_user_id'],$auth_user_ids) && $isDel) {
                $isDel = false;
            ...
            # 检查合同下是否有回款
            if ($isDel) {
                if (db('crm_receivables')->where(['contract_id' => $v, 'check_status' => ['in', '0,1,2']])->value('receivables_id')) {
                    $isDel = false;
                    $errorMessage[] = '名称为'.$data['name'].'的合同删除失败,错误原因：当前合同下有回款，不可删除！';
                }                
            }
            if ($isDel) {
                $delIds[] = $v;
...
//            }
    ...
    public function system()
    {
        ...
        $data = $contractModel->getSystemInfo($this->param['id']);
    ...
    {
        if (empty($this->param['contract_id'])) return resultArray(['error' => '参数错误！']);
        ...
        # 团队
        $contract['ro_user_id']    = explode(',', trim($contract['ro_user_id'], ','));
        $contract['rw_user_id']    = explode(',', trim($contract['rw_user_id'], ','));
        ...
        # 台账
        $ledgerCount = Db::name('customer_ledger')->where('contract_id', $contractId)->count();

        # 收支流水
        $financeCount = Db::name('finance_record')->where('contract_id', $contractId)->count();
    ...
    public function copy()
    {
        ...
        if (!$contractModel->copy($contractId, $userId)) return resultArray(['error' => '操作失败！']);
