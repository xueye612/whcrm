...
//        }
    ...
    public function count()
    {
        ...
        $businessQuery = Db::name('crm_business')->where('customer_id', $customerId)->where($businessAuth);
        $businessCount = $businessQuery->count();
        $businessIds = Db::name('crm_business')->where('customer_id', $customerId)->where($businessAuth)->column('business_id');

        # 合同
        $contractAuth = $this->getContractSearchWhere($userInfo['id']);
        $contractQuery = Db::name('crm_contract')->where('customer_id', $customerId)->where($contractAuth);
        $contractCount = $contractQuery->count();
        $contractIds = Db::name('crm_contract')->where('customer_id', $customerId)->where($contractAuth)->column('contract_id');
        ...
        # 台账 - 包含客户本身 + 客户下商机/合同
        $ledgerQuery = Db::name('customer_ledger')->where('customer_id', $customerId);
        if (!empty($businessIds)) {
            $ledgerQuery->whereOr('business_id', 'in', $businessIds);
        }
        ...
        # 收支流水 - 包含客户本身 + 客户下商机/合同
        $financeQuery = Db::name('finance_record')->where('customer_id', $customerId);
        if (!empty($businessIds)) {
            $financeQuery->whereOr('business_id', 'in', $businessIds);
        }
        if (!empty($contractIds)) {
